...
if (formData) {
    ...
    // Cria a celula Relatorio
    const relatorioCell = newRow.insertCell();
    const relatorioButton = document.createElement('button');
    relatorioButton.textContent = 'Relatório';
    relatorioButton.addEventListener('click', openRelatorioModal);
    relatorioCell.appendChild(relatorioButton);
...
function enviarRelatorio() {
    const texto = document.getElementById('relatorioTextarea').value;
    if (texto.trim() === '') {
        alert("Por favor, escreva o relatório antes de enviar.");
        return;
    }
    // Aqui você pode fazer um POST para o servidor ou salvar no localStorage
    console.log("Relatório enviado:", texto);
